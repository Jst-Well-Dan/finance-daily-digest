...
7. 主 Agent 真实阅读全部结构化笔记的来源元信息与“核心结论”；对核心结论中关键、矛盾、含糊或高影响判断回查原始转写稿，并用标题、视频 ID、发布日期和来源 URL 核验来源。
8. 按下面的“固定每日总结结构”撰写 `<daily_dir>/YYYYMMDD/YYYYMMDD_解读君视频总结.md`。
...
- 完整阅读转写稿。清楚区分“已核验事实”“视频中的事实性陈述（待核验）”“嘉宾观点”和“Agent 对其逻辑的整理”；不得将后两者写成事实。
...
- 转写中的人名、公司名、金额、百分比、政策条文和时间表优先回看原视频或一手文件；发现语义突变、疑似串入文本或转写错误，须写入“转写质量问题”并从核心结论中剔除。
...
每日总结只包含每篇视频的标题和结构化笔记的“核心结论”。不得加入统计范围、来源元信息、跨视频综合结论、主题重组、后续跟踪清单、投资研究指引、参考视频段落或其他内容。
